...
class Application {
	...
	public function __construct() {
		$this->plugin_name         = 'combidesk-exact';
		$this->plugin_product_name = __( 'Exact Online', $this->plugin_name );
		$this->title               = sprintf( __( 'Combidesk - %s for WooCommerce', $this->plugin_name ), $this->plugin_product_name );
		$this->menu_title          = sprintf( __( 'Combidesk - %s', $this->plugin_product_name ), $this->plugin_product_name );
		
		//Dummy literals can't have sprintf!
		$this->description = __( 'Synchronize your WooCommerce orders automatically to Exact Online', $this->plugin_name );
		$this->run();
	}
	...
	public function woo_hpos_compatibility() {
		if ( class_exists( \Automattic\WooCommerce\Utilities\FeaturesUtil::class ) ) {
			\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', 'combidesk-exact/combidesk-exact.php', true );
		}
	}
	...
	function add_combidesk_page() {
		...
		$menu_slug  = 'combidesk-exact-settings';
		...
		echo '
		  <h1>' . esc_html( $this->title ) . '</h1>
			...
			<ul>
				...
				<li>' . __( 'Completed orders will be transferred automatically during installation.', $this->plugin_name ) . '</li>
				<li>' . __( 'Order history can be automatically transferred.', $this->plugin_name ) . '</li>
				<li>' . __( 'Orders can be created in Exact Online as sales entry, sales invoice or sales order.', 'combidesk-exact' ) . '</li>
				<li>' . sprintf( __( 'Customers will be generated in %s based on their unique e-mail address.', $this->plugin_name ), $this->plugin_product_name ) . '</li>
		  ...
		  <form class="combidesk__cta-form" action="https://combidesk.com/integration/woocommerce-exactonline" method="GET" target="_BLANK">
		  ...
		  <form class="combidesk__cta-form" action="https://www.exact.com/nl/producten/boekhouden" method="GET" target="_BLANK">
			<input type="hidden" name="email_address" value="' . esc_attr( $email_address ) . '" />
